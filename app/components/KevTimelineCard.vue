...
const aligners: Record<Period, (date: Date) => Date> = {
  daily: startOfDay,
  weekly: (date: Date) => startOfWeek(date, weekOptions),
...
watch(
  () => [props.period, props.range.start, props.range.end, props.entries],
  () => {
    ...
    const dates = props.period === 'weekly'
      ? (eachWeekOfInterval({ start, end }, weekOptions) as Date[])
      : generator({ start, end })
...
const template = (datum: DataRecord) => `${formatDate(datum.date)}: ${formatNumber(datum.amount)}`
</script>
