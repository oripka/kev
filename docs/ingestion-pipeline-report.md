...
Metasploit modules are especially prone to this issue because our importer guesses vendor/product pairs from a mix of module metadata, platform tags, and repository paths. The `guessVendorProduct` helper scans the module’s name, description, targets, aliases, and relative path for platform keywords. Any token such as `windows`, whether found in `metadata.platforms`, the `modules/exploits/windows/…` directory structure, or the target list, maps straight to `Microsoft`/`Windows` via the `PLATFORM_VENDOR_MAP` lookup before normalisation occurs.【F:server/utils/metasploit.ts†L1058-L1139】【F:server/utils/metasploit.ts†L822-L842】 That heuristic is intentionally broad so Linux, macOS, and other operating-system families also collapse to canonical vendors when the module omits a clear product name.【F:server/utils/metasploit.ts†L1125-L1139】
