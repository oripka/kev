...
3. Merge cached CVSS metrics with any new scores retrieved via NVD, then enrich each base entry with CVEList impacts and our domain/exploit/vulnerability classifiers. The CVEList pass prefers the primary vendor/product extracted from CNA or CPE applicability nodes when they exist—overwriting the normaliser’s fallbacks—and records the full affected-products matrix so downstream analytics inherit authoritative labels rather than guesses.【F:server/api/fetchKev.post.ts†L283-L552】【F:server/utils/cvelist.ts†L262-L339】【F:server/utils/cvelist-parser.ts†L469-L508】
...
Every importer funnels through `normaliseVendorProduct`, which defaults ambiguous values to "Unknown," applies a finite set of regex overrides, and only infers vendors from a handful of keyword patterns (e.g., Windows → Microsoft, iOS/macOS → Apple).【F:app/utils/vendorProduct.ts†L1-L139】 When upstream feeds provide sparse strings—or when products lack Microsoft/Apple-style keywords—the helper cannot confidently assign the right vendor or family, leading to the inaccurate catalog labels we observe across sources. CVEList enrichment can later override those placeholders when CNA or CPE metadata supplies authoritative vendor/product pairs, but it cannot fix records where the upstream CVE also omits them.【F:server/utils/cvelist.ts†L262-L339】【F:server/utils/cvelist-parser.ts†L469-L508】
...
Metasploit modules are especially prone to this issue because our importer guesses vendor/product pairs from a mix of module metadata, platform tags, and repository paths. The `guessVendorProduct` helper still inspects the module’s name, description, targets, aliases, and relative path for hints, but we no longer consult the old platform vendor map for signals that originate solely from repository paths or `metadata.platforms`.【F:server/utils/metasploit.ts†L1038-L1107】 Tokens such as `windows` appearing only in those locations no longer auto-promote a `Microsoft` label; the heuristics now fall back to `Unknown` unless a richer hint exists.
...
The result had been a systemic bias toward Microsoft (and other platform vendors) for any Metasploit module that runs on Windows, even when the actual vulnerability belongs to Commvault, Pandora, Samsung, or another vendor. To correct this, we removed the repository-path and `metadata.platforms` shortcuts and now only promote platform-derived vendors when the target list also names the vendor (e.g., `Microsoft` alongside `Windows`).【F:server/utils/metasploit.ts†L1080-L1105】 This keeps genuine operating-system exploit modules aligned while preventing platform-only hints from overwriting third-party vendors. We also documented the change here so future investigations know why those heuristics were narrowed.
